    ...
    try {
      if (isSignUp) {
        ...
        const { error } = await supabase.auth.signUp({
          ...
          password,
          options: {
            // Send metadata so the SQL Trigger can pick it up
        ...
        router.refresh(); // Refresh to show avatar
      }
    } catch (err: any) {
      ...
      <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl w-full max-w-md shadow-2xl relative overflow-hidden">
        ...
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-black text-white mb-2">
            {isSignUp ? 'Join the Squad' : 'HQ Login'}
          </h1>
          <p className="text-slate-400 text-sm">
            {isSignUp ? 'Create your profile and join the leaderboard.' : 'Enter your credentials, soldier.'}
          </p>
        </div>
        ...
        <form onSubmit={handleAuth} className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl py-3 pl-10 pr-4 text-white focus:outline-none focus:border-indigo-500 transition"
                />
              </div>
            </div>
          )}
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3 rounded-xl transition flex items-center justify-center gap-2 mt-6"
          >
